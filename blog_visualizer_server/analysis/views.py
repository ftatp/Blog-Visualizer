...
class textclass:
	...
	def HTML_preprocessing(structure_p_img_tag):
		...
		array_temp = Structure_only_tag_df['text'].apply(lambda x: " img ".join(list(map(lambda x: 'text' if len(x) > 3 else '', x.split('<img>')))).strip().replace('  ', ' ')).values
		refined_structure = ''.join(array_temp)

		HTML_preprocessing = {'Text': Text, 'refined_structure': refined_structure, 'Count_space_mistake': Count_space_mistake}
	...
	def sentimental_analysis(text):
		...
		if text == '':
			sentiment_dict = {'pos_ratio': pos_ratio, 'neg_ratio': neg_ratio, 'subjectivity': subjectivity, 'polarity': polarity, 'senti_diffs_per_ref': senti_diffs_per_ref}
...
def get_naver_post_all_data():#User_id, Post_id, Category):
	...
	loaded_vec = TfidfVectorizer(decode_error="replace",vocabulary=pickle.load(open("static/[Structure_tag]TFIDF_features_100.pkl", "rb")))
	transformer = TfidfTransformer()
...
def index(request):
	if request.method == "POST":
		...
		post_data = get_naver_post_all_data()#User_id, Post_id, Category)
